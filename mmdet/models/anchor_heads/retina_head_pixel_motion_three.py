...
from .anchor_head_three import AnchorHeadThree
...
class RetinaHeadPixelMotion_three(AnchorHeadThree):
    ...
    def __init__(self,
        ...
        super(RetinaHeadPixelMotion_three, self).__init__(
    ...
    def _init_layers(self):
        ...
        self.depth_convs = nn.Conv2d(self.in_channels*3, self.in_channels, kernel_size=3, padding=1)
    ...
    def forward_single(self, x_pixel, x_motion, feat):
        # pdb.set_trace()
        x_motion = self.relu(self.depth_states(x_motion))
        feats = torch.cat([x_pixel, x_motion, feat], 1)
        feats = self.relu(self.depth_convs(feats))
        # cls_feat = self.relu(self.red_convs(feats))
        cls_feat = feats
        reg_feat = cls_feat
