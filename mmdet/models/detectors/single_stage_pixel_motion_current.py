...
class SingleStageDetectorPixelMotion_current(BaseDetectorTemp):
    ...
    def __init__(self,
                 ...
                 corr_size=None,
                 gating_seq_len=None):
        super(SingleStageDetectorPixelMotion_current, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(SingleStageDetectorPixelMotion_current, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        for i in range(seq_len-1):
            multi_init_state, attention_feat = self.motion(multi_init_state, x_all[i], x_all[-1])
            attention_feats.append(attention_feat)

        x_motion = [multi_init_state[i][0] for i in range(len(x_all[0]))]

        x_pixel = self.pixel([x_all[-1], attention_feats[0], attention_feats[1], attention_feats[2]])

        outs = self.bbox_head(x_pixel, x_motion)
