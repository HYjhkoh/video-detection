...
class SingleStageDetectorSplit(BaseDetectorTemp):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(SingleStageDetectorSplit, self).__init__()
        self.backbone = builder.build_backbone(backbone)
        if neck is not None:
            self.neck = builder.build_neck(neck)
        self.temporal = builder.build_temporal(temporal)
        self.bbox_head = builder.build_head(bbox_head)
        self.bbox_head_temp = builder.build_head(bbox_head_temp)
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self.init_weights(pretrained=pretrained)

    def init_weights(self, pretrained=None):
        super(SingleStageDetectorSplit, self).init_weights(pretrained)
        ...
        self.bbox_head_temp.init_weights(pretrained=None)
    ...
    def forward_train(self,
        ...
        outs_pre = self.bbox_head(x_pre)
        x_cur = self.extract_feat(img)
        outs_cur = self.bbox_head(x_cur)
        x_temp = self.temporal(outs_pre, outs_cur)
        outs = self.bbox_head_temp(x_temp)
