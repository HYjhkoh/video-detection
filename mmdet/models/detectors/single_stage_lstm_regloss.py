...
class SingleStageDetectorLSTM_regloss(BaseDetectorTemp):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 cur=None):
        super(SingleStageDetectorLSTM_regloss, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(SingleStageDetectorLSTM_regloss, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        outs1 = self.bbox_head(x_temp, x_all[-1])
        outs2 = self.bbox_head2(x_temp)
        loss_inputs1 = outs1 + (gt_bboxes, gt_labels, img_metas, self.train_cfg)
        losses1 = self.bbox_head.loss(
            *loss_inputs1, gt_bboxes_ignore=gt_bboxes_ignore)
        loss_inputs2 = outs2 + (gt_bboxes, gt_labels, img_metas, self.train_cfg)
        losses2 = self.bbox_head2.loss(
            *loss_inputs2, gt_bboxes_ignore=gt_bboxes_ignore)
        losses1['loss_cls'][0] = losses1['loss_cls'][0] + losses2['loss_cls'][0]
        losses1['loss_bbox'][0] = losses1['loss_bbox'][0] + losses2['loss_bbox'][0]
        return losses1
