...
class SingleStageDetectorLSTM_freeze(BaseDetectorTemp):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 cur=None):
        super(SingleStageDetectorLSTM_freeze, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(SingleStageDetectorLSTM_freeze, self).init_weights(pretrained)
        ...
        for i in range(len(self.temporal.x_convolution)):
            if isinstance(self.temporal.x_convolution[i], nn.Conv2d):
                self.temporal.x_convolution[i].weight.requires_grad = False
                self.temporal.x_convolution[i].bias.requires_grad = False

        for i in range(len(self.temporal.h_convolution)):
            if isinstance(self.temporal.h_convolution[i], nn.Conv2d):
                self.temporal.h_convolution[i].weight.requires_grad = False
                self.temporal.h_convolution[i].bias.requires_grad = False        
    ...
    def forward_train(self,
        ...
        for i in range(len(x_all[0])):
            ...
            init_state = (Variable(torch.zeros(batch_size, channel, height, width)).cuda(), Variable(torch.zeros(batch_size, channel, height, width)).cuda())
        ...
        outs = self.bbox_head(x_temp, x_all[-1])
