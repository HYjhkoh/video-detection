...
class SingleStageDetectorCorr_resize(BaseDetectorTemp):
    ...
    def __init__(self,
                 ...
                 in_channels=None,
                 corr_size=None):
        super(SingleStageDetectorCorr_resize, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(SingleStageDetectorCorr_resize, self).init_weights(pretrained)
    ...
    def feature_vis(self, img, features):
        ...
        for idx in range(len(features)):
            ...
            cv2.imwrite('./vis_feature/corr_resize/feature_lstmout_%d.png'%(idx),layer_tot_ori)
            # layer_con_ori.append(const_ori)
        # layer_con_ori = np.concatenate(layer_con_ori,axis=1)
        # layer_tot_ori.append(layer_con_ori)
        # layer_tot_ori = np.concatenate(layer_tot_ori,axis=0)
        ...
        cv2.imwrite('./vis_feature/corr_resize/input_image1.png', inputs[0])
        cv2.imwrite('./vis_feature/corr_resize/input_image2.png', inputs[1])
        cv2.imwrite('./vis_feature/corr_resize/input_image3.png', inputs[2])
        cv2.imwrite('./vis_feature/corr_resize/input_image4.png', inputs[3])
        pdb.set_trace()
    ...
    def forward_train(self,
        ...
        for i in range(len(self.in_channels)):
            ...
            init_state = (Variable(torch.zeros(batch_size, self.corr_size*self.corr_size, height, width)).cuda(),\
                          Variable(torch.zeros(batch_size, self.corr_size*self.corr_size, height, width)).cuda())
        ...
        outs = self.bbox_head(multi_init_state, x_all[-1])
