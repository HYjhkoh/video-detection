...
class SingleStageDetectorLSTM_l1loss_freeze(BaseDetectorTemp):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 cur=None):
        super(SingleStageDetectorLSTM_l1loss_freeze, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(SingleStageDetectorLSTM_l1loss_freeze, self).init_weights(pretrained)
        ...
        for i in range(len(self.backbone.features)):
            if isinstance(self.backbone.features[i], nn.Conv2d):
                self.backbone.features[i].weight.requires_grad = False
                self.backbone.features[i].bias.requires_grad = False

        for i in range(len(self.backbone.extra)):
            self.backbone.extra[i].weight.requires_grad = False
            self.backbone.extra[i].bias.requires_grad = False
    ...
    def forward_train(self,
        ...
        loss_inputs = outs + (gt_bboxes, gt_labels, img_metas, self.train_cfg, x_all[-1], x_temp)
