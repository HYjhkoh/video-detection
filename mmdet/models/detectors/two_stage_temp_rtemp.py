...
from .test_mixins_gating_rgating import RPNTestMixin, BBoxTestMixin, MaskTestMixin
from .. import builder
...
class TwoStageDetectorTempRTemp(BaseDetector, RPNTestMixin, BBoxTestMixin,
                       MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(TwoStageDetectorTempRTemp, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(TwoStageDetectorTempRTemp, self).init_weights(pretrained)
    ...
    def feature_vis(self, img, features):
        ...
        for idx in range(len(features)):
            ...
            cv2.imwrite('./vis_feature/faster/feature_lstminput_%d.png'%(idx),layer_tot_ori)
            # layer_con_ori.append(const_ori)
        # layer_con_ori = np.concatenate(layer_con_ori,axis=1)
        # layer_tot_ori.append(layer_con_ori)
        # layer_tot_ori = np.concatenate(layer_tot_ori,axis=0)
        ...
        cv2.imwrite('./vis_feature/faster/input_image1.png', inputs[0])
        cv2.imwrite('./vis_feature/faster/input_image2.png', inputs[1])
        cv2.imwrite('./vis_feature/faster/input_image3.png', inputs[2])
        cv2.imwrite('./vis_feature/faster/input_image4.png', inputs[3])
        pdb.set_trace()
    ...
    def forward_train(self,
        ...
        x = self.temporal(x_all)

        # if True:
        #     self.feature_vis(img, x_all[-1]) ## x_all[3], feats, x_temp
        ...
        if self.with_bbox:
            ...
            for i in range(seq_len):
                roi_feat = self.bbox_roi_extractor(
                    x_all[i][:self.bbox_roi_extractor.num_inputs], rois)
                roi_feats.append(roi_feat)

            bbox_feats = self.temporal2(roi_feats)
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        det_bboxes, det_labels = self.simple_test_bboxes(
            x_all, img_meta, proposal_list, self.test_cfg.rcnn, rescale=rescale)
