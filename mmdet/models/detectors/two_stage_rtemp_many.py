...
from .test_mixins_rgating import RPNTestMixin, BBoxTestMixin, MaskTestMixin
from .. import builder
...
class TwoStageDetectorRTempMany(BaseDetector, RPNTestMixin, BBoxTestMixin,
                       MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(TwoStageDetectorRTempMany, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(TwoStageDetectorRTempMany, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_rpn:
            ...
            rpn_out_cur = self.rpn_head(x_all[cur_idx])
            ...
            rpn_loss_inputs = rpn_out_cur + (gt_bboxes, img_meta,
                                              self.train_cfg.rpn)
            ...
            proposal_lists = []
            for seq_idx in range(seq_len):
                proposal_inputs = rpn_outs[seq_idx] + (img_meta, proposal_cfg)
        ...
        if self.with_bbox:
            ...
            bbox_feats = self.temporal(roi_feats)
