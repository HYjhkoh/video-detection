...
def build_assigner(cfg, **kwargs):
    if cfg['type'] == 'OHEMSamplerFixed':
        cfg['type'] = 'OHEMSampler'
...
def build_sampler_fixed(cfg, **kwargs):
    if cfg['type'] == 'OHEMSampler':
        cfg['type'] = 'OHEMSamplerFixed'
    if isinstance(cfg, samplers.BaseSamplerFixed):
        return cfg
    elif isinstance(cfg, dict):
        return mmcv.runner.obj_from_dict(cfg, samplers, default_args=kwargs)
    else:
